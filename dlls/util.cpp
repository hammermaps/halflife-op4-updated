...
CBaseEntity* UTIL_FindEntityForward(CBaseEntity* pMe)
{
	...
	UTIL_TraceLine(pMe->pev->origin + pMe->pev->view_ofs,
	               pMe->pev->origin + pMe->pev->view_ofs + gpGlobals->v_forward * 8192, dont_ignore_monsters,
	               pMe->edict(), &tr);
...
void UTIL_ClientPrintAll(int msg_dest, const char* msg_name, const char* param1, const char* param2, const char* param3,
                         const char* param4)
{
...
void ClientPrint(entvars_t* client, int msg_dest, const char* msg_name, const char* param1, const char* param2,
...
{
	TRACE_LINE(vecStart, vecEnd, (igmon == ignore_monsters ? TRUE : FALSE) | (ignoreGlass ? 0x100 : 0), pentIgnore,
	           ptr);
}
...
void UTIL_SetEdictOrigin(entvars_t* pev, const Vector& vecOrigin)
{
	edict_t* ent = ENT(pev);
	if (ent)
...
{
	UTIL_SetEdictOrigin(pEntity->pev, vecOrigin);
}

void UTIL_SetEntvarsOrigin(entvars_t* pEntvars, const Vector& vecOrigin)
{
	UTIL_SetEdictOrigin(pEntvars, vecOrigin);
}
...
void CSave::WriteFunction(const char* pname, void** data, int count)
{
	...
	if (functionName)
		...
		ALERT(at_error, "Member \"%s\" contains an invalid function pointer %p!", pname, *data);
}
...
int CRestore::ReadField(void* pBaseData, TYPEDESCRIPTION* pFields, int fieldCount, int startField, int size,
                        char* pName, void* pData)
{
...
int CRestore::ReadFields(const char* pname, void* pBaseData, TYPEDESCRIPTION* pFields, int fieldCount)
{
	...
	for (i = 0; i < fileCount; i++)
	{
		...
		lastField = ReadField(pBaseData, pFields, fieldCount, lastField, header.size, m_pdata->pTokens[header.token],
		                      header.pData);
		lastField++;
	}
