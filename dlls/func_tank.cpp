...
void CFuncTank::Spawn()
{
	...
	{
		m_pFireProxy = GetClassPtr(static_cast<CPointEntity*>(nullptr));
	}
}
...
int CFuncTank::IRelationship(CBaseEntity* pTarget)
{
	...
	if (m_iTankClass == CLASS_PLAYER_ALLY)
	{
		switch (iOtherClass)
		{
	...
	if (m_iTankClass == CLASS_HUMAN_MILITARY)
	{
		switch (iOtherClass)
		{
		case CLASS_PLAYER:
	...
	if (m_iTankClass == CLASS_ALIEN_MILITARY)
	{
		switch (iOtherClass)
		{
		case CLASS_PLAYER:
	...
	if (pController)
	{
		...
		if (pController->pev->button & IN_ATTACK)
		{
			...
			if (pController && pController->IsPlayer())
				static_cast<CBasePlayer*>(pController)->m_iWeaponVolume = LOUD_GUN_VOLUME;

			m_flNextAttack = gpGlobals->time + (1 / m_fireRate);
		}
	}
		// firing with automatic guns:
	else if (CanFire() && ((fabs(distX) < m_pitchTolerance && fabs(distY) < m_yawTolerance) || (pev->spawnflags &
		SF_TANK_LINEOFSIGHT)))
	{
		BOOL fire = FALSE;
		Vector forward;
...
void CFuncTank::Fire(const Vector& barrelEnd, const Vector& forward, entvars_t* pevAttacker)
{
	...
	if (m_fireLast != 0)
	{
		if (m_iszSpriteSmoke)
		{
			...
			pSprite->SetTransparency(kRenderTransAlpha, pev->rendercolor.x, pev->rendercolor.y, pev->rendercolor.z, 255,
			                         kRenderFxNone);
		...
		if (bulletCount > 0)
		{
			for (i = 0; i < bulletCount; i++)
			{
				...
				case TANK_BULLET_9MM:
					FireBullets(1, barrelEnd, forward, gTankSpread[m_spread], 4096, BULLET_MONSTER_9MM, 1,
					            m_iBulletDamage, pevAttacker);
					break;

				case TANK_BULLET_MP5:
					FireBullets(1, barrelEnd, forward, gTankSpread[m_spread], 4096, BULLET_MONSTER_MP5, 1,
					            m_iBulletDamage, pevAttacker);
					break;

				case TANK_BULLET_12MM:
					FireBullets(1, barrelEnd, forward, gTankSpread[m_spread], 4096, BULLET_MONSTER_12MM, 1,
					            m_iBulletDamage, pevAttacker);
					break;
...
CLaser* CFuncTankLaser::GetLaser()
{
	...
	while (pEntity)
	{
		...
		{
			m_pLaser = static_cast<CLaser*>(pEntity);
			break;
		}
...
void CFuncTankControls::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (!m_pController && useType != USE_OFF)
	{
		...
		if (m_active != FALSE || static_cast<CBasePlayer*>(pActivator)->m_pTank != nullptr)
			return;
		...
		while (tryTank = UTIL_FindEntityByTargetname(tryTank, STRING(pev->target)))
		{
			...
			{
				if (static_cast<CFuncTank*>(tryTank)->StartControl(static_cast<CBasePlayer*>(pActivator), this))
				{
		...
		if (m_active)
		{
			...
			m_pController = static_cast<CBasePlayer*>(pActivator);
...
void CTankSequence::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (GetState() == STATE_OFF)
	{
		...
		CFuncTank* pTank = static_cast<CFuncTank*>(pEnt);
