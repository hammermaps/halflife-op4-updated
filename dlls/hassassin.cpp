	...
	case ASSASSIN_AE_TOSS1:
		{
			...
			{
				Vector vecToss;
				if (m_pCine->PreciseAttack() && HasTargetEntity())
				{
...
{
	if ( m_flNextJump < gpGlobals->time && (flDist <= 128 || HasMemory( bits_MEMORY_BADJUMP )) && HasEnemy())
	{
		TraceResult	tr;
...
void CHAssassin :: RunAI()
{
	...
	if (g_iSkillLevel != SKILL_HARD || !HasEnemy() || pev->deadflag != DEAD_NO || m_Activity == ACT_RUN || m_Activity == ACT_WALK || !(pev->flags & FL_ONGROUND))
