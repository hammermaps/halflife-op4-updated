...
public:
	...
	bool AcceptableSpeaker(CBaseMonster* pMonster) const;
...
auto CScriptedSentence::AcceptableSpeaker(CBaseMonster* pMonster) const -> bool
{
	if (pMonster)
	{
		if (pev->spawnflags & SF_SENTENCE_FOLLOWERS)
		{
			if (!pMonster->HasTargetEntity() || !FClassnameIs(pMonster->m_hTargetEnt->pev, "player"))
				return false;
		}

		if (pMonster->CanPlaySentence(pev->spawnflags & SF_SENTENCE_INTERRUPT))
			return true;
	}
...
{
	CBaseEntity* pTarget = UTIL_FindEntityByTargetname(nullptr, STRING(m_iszEntity), pActivator);
...
auto CScriptedSentence::StartSentence(CBaseMonster* pTarget) -> bool
{
	if (!pTarget)
	{
	...
	pTarget->PlayScriptedSentence(STRING(m_iszSentence), m_flDuration, m_flVolume, m_flAttenuation, !(pev->spawnflags & SF_SENTENCE_CONCURRENT), pListener);
