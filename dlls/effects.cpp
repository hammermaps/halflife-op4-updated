...
CBeam* CBeam::BeamCreate(const char* pSpriteName, int width)
{
	...
	CBeam* pBeam = GetClassPtr(static_cast<CBeam*>(nullptr));
...
CBaseEntity* CBeam::GetTripEntity(TraceResult* ptr)
{
	...
	if (FClassnameIs(pTrip->pev, STRING(pev->netname)))
		return pTrip;
	if (FStrEq(STRING(pTrip->pev->targetname), STRING(pev->netname)))
		return pTrip;
	return nullptr;
}
...
void CBeam::BeamDamage(TraceResult* ptr)
{
	...
	if (ptr->flFraction != 1.0 && ptr->pHit != nullptr)
	{
		...
		if (pHit)
		{
			...
			{
				ClearMultiDamage();
				pHit->TraceAttack(pev, pev->dmg * (gpGlobals->time - pev->dmgtime),
				                  (ptr->vecEndPos - pev->origin).Normalize(), ptr, pev->frags);
...
void CLaser::PostSpawn()
{
	if (m_iszStartSpriteName)
	{
		...
		if (pTemp == nullptr)
		{
			...
			if (m_pStartSprite)
				m_pStartSprite->SetTransparency(kRenderGlow, pev->rendercolor.x, pev->rendercolor.y, pev->rendercolor.z,
		...
		else
		{
			...
			m_pStartSprite = static_cast<CSprite*>(pTemp);
	...
	if (m_iszEndSpriteName)
	{
		...
		if (pTemp == nullptr)
		{
			...
			if (m_pEndSprite)
				m_pEndSprite->SetTransparency(kRenderGlow, pev->rendercolor.x, pev->rendercolor.y, pev->rendercolor.z,
				                              pev->renderamt, pev->renderfx);
		}
		else if (!FClassnameIs(pTemp->pev, "env_sprite"))
		{
		...
		{
			// use an env_sprite defined by the mapper
			m_pEndSprite = static_cast<CSprite*>(pTemp);
...
void CLaser::StrikeThink()
{
	...
	{
		startpos = CalcLocus_Position(this, nullptr, STRING(m_iszStartPosition));
	}

	if (m_iTowardsMode)
	{
		m_firePosition = startpos + CalcLocus_Velocity(this, nullptr, STRING(pev->message));
	}
	else
	{
...
{
	CSprite* pSprite = GetClassPtr(static_cast<CSprite*>(nullptr));
		...
		if (pGib)
		{
			...
			if (pev->spawnflags & SF_GIBSHOOTER_DEBUG)
				ALERT(at_console, "DEBUG: %s \"%s\" creates a shot at %f %f %f; vel %f %f %f; pos \"%s\"\n",
				      STRING(pev->classname), STRING(pev->targetname), pGib->pev->origin.x, pGib->pev->origin.y,
				      pGib->pev->origin.z, pGib->pev->velocity.x, pGib->pev->velocity.y, pGib->pev->velocity.z,
...
void CEnvShooter::KeyValue(KeyValueData* pkvd)
{
	...
	{
		UTIL_StringToVector(static_cast<float*>(m_vecSize), pkvd->szValue);
...
CBaseEntity* CEnvShooter::CreateGib(Vector vecPos, Vector vecVel)
{
	...
	// special shot
	CShot* pShot = GetClassPtr(static_cast<CShot*>(nullptr));
	...
	{
		pShot->m_maxFrame = static_cast<float>(MODEL_FRAMES(pShot->pev->modelindex)) - 1;
...
Vector CBlood::BloodPosition(CBaseEntity* pActivator)
{
	if (pev->spawnflags & SF_BLOOD_PLAYER)
	{
		...
		if (pPlayer)
			return (pPlayer->v.origin + pPlayer->v.view_ofs) + Vector(
				RANDOM_FLOAT(-10, 10), RANDOM_FLOAT(-10, 10), RANDOM_FLOAT(-10, 10));
...
void CBlood::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	if (pev->spawnflags & SF_BLOOD_STREAM)
		UTIL_BloodStream(BloodPosition(pActivator), Direction(pActivator), (Color() == BLOOD_COLOR_RED) ? 70 : Color(),
		                 BloodAmount());
	else
...
void CEnvFootsteps::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (pActivator && pActivator->IsPlayer())
	{
		...
		if (pev->spawnflags & SF_FOOTSTEPS_SET || (!(pev->impulse & playerMask) && (useType == USE_ON || useType ==
			USE_TOGGLE)))
		{
			...
			{
				char sTemp[4];
				sprintf(sTemp, "%d", static_cast<int>(pev->frags));
...
void CEnvRain::Think()
{
	...
	while (drawn < repeats && tries < (repeats * 3))
	{
		...
		WRITE_COORD(vecSrc.z);
		WRITE_SHORT(m_spriteTexture);
		WRITE_BYTE(static_cast<int>(0)); // framestart
		WRITE_BYTE(static_cast<int>(0)); // framerate
		if (m_fLifeTime) // life
			WRITE_BYTE(static_cast<int>(m_fLifeTime * 10));
		else if (m_flMaxUpdateTime)
			WRITE_BYTE(static_cast<int>((RANDOM_FLOAT(m_flUpdateTime, m_flMaxUpdateTime) * 30)));
		else
			WRITE_BYTE(static_cast<int>(m_flUpdateTime * 30)); // life
		WRITE_BYTE(m_dripSize); // width
		WRITE_BYTE(m_iNoise); // noise
		WRITE_BYTE(static_cast<int>(pev->rendercolor.x)); // r,
		WRITE_BYTE(static_cast<int>(pev->rendercolor.y)); //    g,
		WRITE_BYTE(static_cast<int>(pev->rendercolor.z)); //       b
		WRITE_BYTE(m_brightness); // brightness
		WRITE_BYTE(static_cast<int>(RANDOM_LONG(m_minDripSpeed, m_maxDripSpeed))); // speed
		MESSAGE_END();
	}
...
{
	MESSAGE_BEGIN(MSG_ALL, gmsgKeyedDLight, nullptr);
	WRITE_BYTE(m_iKey);
...
void CEnvBeverage::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	CBaseEntity* pCan = Create("item_sodacan", vecPos, pev->angles, edict());
...
public:
	void Activate() override;
	void Think() override;
	void Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value) override; // Windawz
};
