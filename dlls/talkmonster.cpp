...
CBaseEntity* CTalkMonster::FindNearestFriend(bool fPlayer)
{
	...
	for (int i = cfriends - 1; i > -1; i--)
	{
		...
		while (pFriend = UTIL_FindEntityByClassname(pFriend, pszFriend))
		{
			...
			if (!pMonster || pMonster->m_MonsterState == MONSTERSTATE_SCRIPT || pMonster->m_MonsterState ==
				MONSTERSTATE_PRONE)
				continue;
...
auto CTalkMonster::FOkToSpeak() -> bool
{
...
auto CTalkMonster::CanPlaySentence(bool fDisregardState) -> bool
{
	if (fDisregardState)
...
// FIdleStare
//=========================================================
auto CTalkMonster::FIdleStare() -> bool
{
	if (!FOkToSpeak())
		return false;
...
void CTalkMonster::PlayScriptedSentence(const char* pszSentence, float duration, float volume, float attenuation,
...
auto CTalkMonster::CanFollow() -> bool
{
