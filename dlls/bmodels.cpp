...
{
	return (pevBModel->absmin + pevBModel->absmax) * 0.5; //LRC - Bug fix for rotating entitys
}
...
public:
	...
	int ObjectCaps() override // Bmodels don't go across transitions
	{
...
void CFuncConveyor::UpdateSpeed(float speed)
{
	...
	int speedCode = static_cast<int>(fabs(speed) * 16.0);
...
void CFuncShine::Think()
{
	...
	MESSAGE_BEGIN(MSG_BROADCAST, gmsgAddShine, nullptr);
...
void CFuncRotating::KeyValue(KeyValueData* pkvd)
{
	...
	{
		Vector tmp;
		UTIL_StringToVector(static_cast<float*>(tmp), pkvd->szValue);
		if (tmp != g_vecZero)
			pev->origin = tmp;
	}
