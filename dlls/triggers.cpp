...
void CMultiManager::ManagerThink()
{
	...
	while (index < m_cTargets && m_flTargetDelay[index] <= time)
	{
		...
		{
			ALERT(at_aiconsole,
			      "WARNING: non-multithreaded multi_manager %s tried triggering itself!\nAvoiding crash...\n",
			      STRING(pev->targetname));
		}
...
void CMultiManager::ManagerUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	if (pev->spawnflags & SF_MULTIMAN_LOOP)
	{
		...
		if (useType == USE_OFF) // it's already off
		{
			if (pev->spawnflags & SF_MULTIMAN_DEBUG)
	...
	else if (FStrEq(pkvd->szKeyName, "m_iszWatch"))
	{
		...
		else
		{
			ALERT(at_console, "%s: Too many targets for %s \"%s\" (limit is %d)\n", pkvd->szKeyName,
			      STRING(pev->classname), STRING(pev->targetname), MAX_MULTI_TARGETS);
		}
	}
	else // add this field to the target list
	{
...
void CRenderFxManager::Affect(CBaseEntity* pTarget, BOOL bIsFirst, CBaseEntity* pActivator)
{
	...
	else
	{
		...
		CRenderFxFader* pFader = GetClassPtr(static_cast<CRenderFxFader*>(nullptr));
...
void CEnvCustomize::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	else
	{
		...
		if (fail && pev->spawnflags & SF_CUSTOM_DEBUG)
			ALERT(at_console,
			      "DEBUG: env_customize \"%s\" does nothing; can't find any entity with name or class \"%s\".\n",
			      STRING(pev->target));
	}
...
void CEnvCustomize::Affect(CBaseEntity* pTarget, USE_TYPE useType)
{
	...
	if (pev->spawnflags & SF_CUSTOM_DEBUG)
		ALERT(at_console, "DEBUG: env_customize \"%s\" affects %s \"%s\": [", STRING(pev->targetname),
	...
	*/
	...
	if (m_voicePitch > 0)
	{
		if (FClassnameIs(pTarget->pev, "monster_barney") || FClassnameIs(pTarget->pev, "monster_scientist") ||
			FClassnameIs(pTarget->pev, "monster_sitting_scientist"))
		{
			static_cast<CTalkMonster*>(pTarget)->m_voicePitch = m_voicePitch;
			if (pev->spawnflags & SF_CUSTOM_DEBUG)
...
void CEnvCustomize::SetBoneController(float fController, int cnum, CBaseEntity* pTarget)
{
	if (fController) //FIXME: the pTarget isn't necessarily a CBaseAnimating.
	{
		if (fController == 1024)
		{
			static_cast<CBaseAnimating*>(pTarget)->SetBoneController(cnum, 0);
			if (pev->spawnflags & SF_CUSTOM_DEBUG)
				ALERT(at_console, " bone%d=0", cnum);
		}
		else
		{
			static_cast<CBaseAnimating*>(pTarget)->SetBoneController(cnum, fController);
			if (pev->spawnflags & SF_CUSTOM_DEBUG)
...
{
	CBasePlayer* pPlayer = static_cast<CBasePlayer*>(Instance(pev->aiment));
	...
	pct = static_cast<int>(static_cast<float>(pPlayer->pev->armorvalue * 100.0) * (1.0 / MAX_NORMAL_BATTERY) + 0.5);
	pct = (pct / 5);
	if (pct > 0)
		pct--;
	...
	static_cast<CBasePlayer*>(pPlayer)->SetSuitUpdate(szcharge, FALSE, SUIT_REPEAT_OK);
}
...
CInOutRegister* CInOutRegister::Add(CBaseEntity* pValue)
{
	...
	CInOutRegister* pResult = GetClassPtr(static_cast<CInOutRegister*>(nullptr));
...
void CTriggerInOut::Spawn()
{
	...
	m_pRegister = GetClassPtr(static_cast<CInOutRegister*>(nullptr));
...
void CTriggerCounter::CounterUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (m_cTriggersLeft != 0)
	{
		if (fTellActivator)
		{
			...
			{
			case 1: ALERT(at_console, "Only 1 more to go...");
				break;
			case 2: ALERT(at_console, "Only 2 more to go...");
				break;
			case 3: ALERT(at_console, "Only 3 more to go...");
				break;
			default: ALERT(at_console, "There are more to go...");
				break;
			}
		}
		return;
...
public:
	...
	static int AddTransitionToList(LEVELLIST* pLevelList, int listCount, const char* pMapName,
...
void CChangeLevel::ChangeLevelNow(CBaseEntity* pActivator)
{
	...
	CBaseEntity* pPlayer = Instance(g_engfuncs.pfnPEntityOfEntIndex(1));
	...
	if (m_changeTarget)
	{
		CFireAndDie* pFireAndDie = GetClassPtr(static_cast<CFireAndDie*>(nullptr));
		if (pFireAndDie)
		{
	...
	while (pChangelevel)
	{
		...
		if (pTrigger)
		{
			...
			if (pentLandmark)
			{
				...
				if (AddTransitionToList(pLevelList, count, pTrigger->m_szMapName, pTrigger->m_szLandmarkName,
				                        pentLandmark))
				{
					count++;
...
void NextLevel()
{
	...
	if (!pEnt)
	{
		...
		pChange = GetClassPtr(static_cast<CChangeLevel*>(nullptr));
...
BOOL CTriggerOnSight::CanSee(CBaseEntity* pLooker, CBaseEntity* pSeen)
{
	...
	if (!FBitSet(pev->spawnflags, SF_ONSIGHT_NOLOS))
	{
		...
		if (SF_ONSIGHT_NOGLASS)
			UTIL_TraceLine(pLooker->EyePosition(), pSeen->pev->origin, ignore_monsters, ignore_glass, pLooker->edict(),
			               &tr);
		else
			UTIL_TraceLine(pLooker->EyePosition(), pSeen->pev->origin, ignore_monsters, dont_ignore_glass,
			               pLooker->edict(), &tr);
...
void CTriggerMotion::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (m_iszPosition)
	{
		...
		case 0:
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Set origin from %f %f %f ", pTarget->pev->origin.x, pTarget->pev->origin.y,
				      pTarget->pev->origin.z);
			pTarget->pev->origin = CalcLocus_Position(this, pActivator, STRING(m_iszPosition));
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", pTarget->pev->origin.x, pTarget->pev->origin.y,
				      pTarget->pev->origin.z);
	...
	if (m_iszAngles)
	{
		...
		case 0:
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Set angles from %f %f %f ", pTarget->pev->angles.x, pTarget->pev->angles.y,
				      pTarget->pev->angles.z);
			pTarget->pev->angles = UTIL_VecToAngles(vecTemp);
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", pTarget->pev->angles.x, pTarget->pev->angles.y,
				      pTarget->pev->angles.z);
			break;
		case 1:
			vecTemp = CalcLocus_Velocity(this, pActivator, STRING(m_iszVelocity));
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Rotate angles from %f %f %f ", pTarget->pev->angles.x, pTarget->pev->angles.y,
				      pTarget->pev->angles.z);
	...
	if (m_iszVelocity)
	{
		...
		case 0:
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Set velocity from %f %f %f ", pTarget->pev->velocity.x,
				      pTarget->pev->velocity.y, pTarget->pev->velocity.z);
			pTarget->pev->velocity = CalcLocus_Velocity(this, pActivator, STRING(m_iszVelocity));
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", pTarget->pev->velocity.x, pTarget->pev->velocity.y,
				      pTarget->pev->velocity.z);
			break;
		case 1:
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Set velocity from %f %f %f ", pTarget->pev->velocity.x,
				      pTarget->pev->velocity.y, pTarget->pev->velocity.z);
			pTarget->pev->velocity = pTarget->pev->velocity + CalcLocus_Velocity(
				this, pActivator, STRING(m_iszVelocity));
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", pTarget->pev->velocity.x, pTarget->pev->velocity.y,
				      pTarget->pev->velocity.z);
			break;
		case 2:
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Rotate velocity from %f %f %f ", pTarget->pev->velocity.x,
				      pTarget->pev->velocity.y, pTarget->pev->velocity.z);
	...
	case 0:
		...
		if (pev->spawnflags & SF_MOTION_DEBUG)
			ALERT(at_console, "DEBUG: Set avelocity from %f %f %f ", pTarget->pev->avelocity.x,
			      pTarget->pev->avelocity.y, pTarget->pev->avelocity.z);
		pTarget->pev->avelocity = vecTemp;
		if (pev->spawnflags & SF_MOTION_DEBUG)
			ALERT(at_console, "to %f %f %f\n", pTarget->pev->avelocity.x, pTarget->pev->avelocity.y,
			      pTarget->pev->avelocity.z);
		break;
	case 1:
		UTIL_StringToRandomVector(vecTemp, STRING(m_iszAVelocity));
		if (pev->spawnflags & SF_MOTION_DEBUG)
...
void CMotionThread::Think()
{
	...
	if (pev->spawnflags & SF_MOTION_DEBUG)
		ALERT(at_console, "motion_thread affects %s \"%s\":\n", STRING(m_hTarget->pev->classname),
		      STRING(m_hTarget->pev->targetname));
	...
	if (m_iszPosition)
	{
		...
		case 0: // set position
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Set origin from %f %f %f ", m_hTarget->pev->origin.x,
				      m_hTarget->pev->origin.y, m_hTarget->pev->origin.z);
			UTIL_AssignOrigin(m_hTarget, CalcLocus_Position(this, m_hLocus, STRING(m_iszPosition)));
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", m_hTarget->pev->origin.x, m_hTarget->pev->origin.y,
				      m_hTarget->pev->origin.z);
			m_hTarget->pev->flags &= ~FL_ONGROUND;
			break;
		case 1: // offset position (= fake velocity)
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Offset origin from %f %f %f ", m_hTarget->pev->origin.x,
				      m_hTarget->pev->origin.y, m_hTarget->pev->origin.z);
			UTIL_AssignOrigin(
				m_hTarget,
				m_hTarget->pev->origin + gpGlobals->frametime * CalcLocus_Velocity(
					this, m_hLocus, STRING(m_iszPosition)));
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", m_hTarget->pev->origin.x, m_hTarget->pev->origin.y,
				      m_hTarget->pev->origin.z);
			m_hTarget->pev->flags &= ~FL_ONGROUND;
		...
		case 2: // set velocity
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Set velocity from %f %f %f ", m_hTarget->pev->velocity.x,
				      m_hTarget->pev->velocity.y, m_hTarget->pev->velocity.z);
			UTIL_SetVelocity(m_hTarget, CalcLocus_Velocity(this, m_hLocus, STRING(m_iszPosition)));
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", m_hTarget->pev->velocity.x, m_hTarget->pev->velocity.y,
				      m_hTarget->pev->velocity.z);
			break;
		case 3: // accelerate
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Accelerate from %f %f %f ", m_hTarget->pev->velocity.x,
				      m_hTarget->pev->velocity.y, m_hTarget->pev->velocity.z);
			UTIL_SetVelocity(
				m_hTarget,
				m_hTarget->pev->velocity + gpGlobals->frametime * CalcLocus_Velocity(
					this, m_hLocus, STRING(m_iszPosition)));
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", m_hTarget->pev->velocity.x, m_hTarget->pev->velocity.y,
				      m_hTarget->pev->velocity.z);
		...
		case 4: // follow position
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Set velocity (path) from %f %f %f ", m_hTarget->pev->velocity.x,
				      m_hTarget->pev->velocity.y, m_hTarget->pev->velocity.z);
			UTIL_SetVelocity(
				m_hTarget, CalcLocus_Position(this, m_hLocus, STRING(m_iszPosition)) - m_hTarget->pev->origin);
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", m_hTarget->pev->velocity.x, m_hTarget->pev->velocity.y,
				      m_hTarget->pev->velocity.z);
			break;
	...
	if (m_iszFacing)
	{
		...
		case 0: // set angles
			...
			{
				if (pev->spawnflags & SF_MOTION_DEBUG)
					ALERT(at_console, "DEBUG: Set angles from %f %f %f ", m_hTarget->pev->angles.x,
					      m_hTarget->pev->angles.y, m_hTarget->pev->angles.z);
				UTIL_SetAngles(m_hTarget, UTIL_VecToAngles(vecTemp));
				if (pev->spawnflags & SF_MOTION_DEBUG)
					ALERT(at_console, "to %f %f %f\n", m_hTarget->pev->angles.x, m_hTarget->pev->angles.y,
					      m_hTarget->pev->angles.z);
			}
		...
		case 1: // offset angles (= fake avelocity)
			...
			{
				if (pev->spawnflags & SF_MOTION_DEBUG)
					ALERT(at_console, "DEBUG: Offset angles from %f %f %f ", m_hTarget->pev->angles.x,
					      m_hTarget->pev->angles.y, m_hTarget->pev->angles.z);
		...
		case 2: // offset angles (= fake avelocity)
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Rotate angles from %f %f %f ", m_hTarget->pev->angles.x,
				      m_hTarget->pev->angles.y, m_hTarget->pev->angles.z);
		...
		case 3: // set avelocity
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "DEBUG: Set avelocity from %f %f %f ", m_hTarget->pev->avelocity.x,
				      m_hTarget->pev->avelocity.y, m_hTarget->pev->avelocity.z);
			UTIL_SetAvelocity(m_hTarget, vecTemp);
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_console, "to %f %f %f\n", m_hTarget->pev->avelocity.x, m_hTarget->pev->avelocity.y,
				      m_hTarget->pev->avelocity.z);
			break;
		}
	}
}
...
void CMotionManager::Affect(CBaseEntity* pTarget, CBaseEntity* pActivator)
{
	...
	CMotionThread* pThread = GetClassPtr(static_cast<CMotionThread*>(nullptr));
	if (pThread == nullptr) return; //error?
...
void COFTriggerGeneWormHit::GeneWormHitTouch(CBaseEntity* pOther)
{
	if (gpGlobals->time - m_flLastDamageTime >= 2 && pOther->pev->takedamage != DAMAGE_NO)
	{
		...
		if (!(pev->spawnflags & SF_GENEWORM_HIT_NO_CLIENTS) || !pOther->IsPlayer())
		{
			...
			EMIT_SOUND_DYN(pOther->edict(), CHAN_BODY, pAttackSounds[RANDOM_LONG(0, ARRAYSIZE(pAttackSounds) - 1)],
			               VOL_NORM, 0.1, 0, RANDOM_LONG(-5, 5) + 100);
...
void CTriggerCTFGeneric::Touch(CBaseEntity* pOther)
{
	if (m_flTriggerDelayTime <= gpGlobals->time)
	{
		...
		if (team_score)
		{
			...
			if (!pOther && !score && pev)
			{
				UTIL_LogPrintf(static_cast<char*>("World triggered \"%s\"\n"), STRING(pev->targetname));
			}
