...
void CGlock::PrimaryAttack()
{
	if (!(m_pPlayer->pev->flags & FL_ONGROUND))
	{
		GlockFire(1.0 * (1 - m_flAccuracy), 0.3f, false);
	}
	else if (m_pPlayer->pev->velocity.Length2D() > 0)
	{
		GlockFire(0.165 * (1 - m_flAccuracy), 0.3f, false);
	}
	else if (m_pPlayer->pev->flags & FL_DUCKING)
	{
		GlockFire(0.075 * (1 - m_flAccuracy), 0.3f, false);
	}
	else
	{
		GlockFire(0.1 * (1 - m_flAccuracy), 0.3f, false);
	}
}
