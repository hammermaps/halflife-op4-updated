...
void CBaseMonster::Look(int iDistance)
{
	...
	ClearConditions(
		bits_COND_SEE_HATE | bits_COND_SEE_DISLIKE | bits_COND_SEE_ENEMY | bits_COND_SEE_FEAR | bits_COND_SEE_NEMESIS |
	...
	if (!FBitSet(pev->spawnflags, SF_MONSTER_PRISONER))
	{
		...
		for (int i = 0; i < count; i++)
		{
				...
				pSightEnt->pev->health > 0)
			{
				...
				if (IRelationship(pSightEnt) != R_NO && FInViewCone(pSightEnt) && !FBitSet(
					pSightEnt->pev->flags, FL_NOTARGET) && FVisible(pSightEnt))
				{
...
void CBaseMonster::MonsterThink()
{
	...
	if (m_MonsterState != MONSTERSTATE_SCRIPT && m_MonsterState != MONSTERSTATE_DEAD && m_Activity == ACT_IDLE &&
		m_fSequenceFinished)
	{
		int iSequence;
...
void CBaseMonster::RouteSimplify(CBaseEntity* pTargetEnt)
{
	...
	for (i = 0; i < count - 1; i++)
	{
		...
		else if (CheckLocalMove(vecStart, m_Route[m_iRouteIndex + i + 1].vecLocation, pTargetEnt, nullptr) ==
			LOCALMOVE_VALID)
		{
			// Skip vert
			continue;
...
int CBaseMonster::CheckLocalMove(const Vector& vecStart, const Vector& vecEnd, CBaseEntity* pTarget, float* pflDist)
{
	...
	*/
	...
	if (iReturn == LOCALMOVE_VALID && !(pev->flags & (FL_FLY | FL_SWIM)) && (!pTarget || (pTarget->pev->flags &
		FL_ONGROUND)))
	{
...
float CBaseMonster::OpenDoorAndWait(entvars_t* pevDoor)
{
	...
	CBaseEntity* pcbeDoor = Instance(pevDoor);
		...
		if (! (m_Route[m_iRouteIndex].iType & bits_MF_IS_GOAL))
		{
				...
				&& (m_Route[m_iRouteIndex + 1].iType & bits_MF_TO_NODE) == bits_MF_TO_NODE)
			{
				...
				if (iLink >= 0 && WorldGraph.m_pLinkPool[iLink].m_pLinkEnt != nullptr)
				{
					...
					if (WorldGraph.HandleLinkEnt(iSrcNode, WorldGraph.m_pLinkPool[iLink].m_pLinkEnt, m_afCapability,
					                             CGraph::NODEGRAPH_DYNAMIC))
					{
...
auto CBaseMonster::BuildRoute(const Vector& vecGoal, int iMoveFlag, CBaseEntity* pTarget) -> bool
{
	...
	if (iLocalMove != LOCALMOVE_INVALID_DONT_TRIANGULATE && FTriangulate(
		pev->origin, vecGoal, flDist, pTarget, &vecApex))
	{
...
//=========================================================
BOOL CBaseMonster::FTriangulate(const Vector& vecStart, const Vector& vecEnd, float flDist, CBaseEntity* pTargetEnt,
                                Vector* pApex)
{
	Vector vecDir;
	Vector vecForward;
			...
			else
			{
				...
				else
				{
					...
					ALERT(at_aiconsole, "%s Failed to move (%d)!\n", STRING(pev->classname),
					      HasMemory(bits_MEMORY_MOVE_FAILED));
...
void CBaseMonster::StartMonster()
{
	...
	if (pev->movetype != MOVETYPE_FLY && !FBitSet(pev->spawnflags, SF_MONSTER_FALL_TO_GROUND))
	{
		...
		{
			ALERT(at_console, "%s \"%s\" stuck in wall--level design error", STRING(pev->classname),
...
BOOL CBaseMonster::FindCover(Vector vecThreat, Vector vecViewOffset, float flMinDist, float flMaxDist)
{
	...
	for (i = 0; i < WorldGraph.m_cNodes; i++)
	{
		...
		{
			UTIL_TraceLine(node.m_vecOrigin + vecViewOffset, vecLookersOffset, ignore_monsters, ignore_glass, ENT(pev),
			               &tr);
			...
			if (tr.flFraction != 1.0)
			{
				...
				if ((iMyNode == iThreatNode) || WorldGraph.PathLength(iMyNode, nodeNumber, iMyHullIndex, m_afCapability)
	...
	for (int i = 0; i < WorldGraph.m_cNodes; i++)
	{
		...
		if (node.m_sHintType)
		{
			...
			if (FValidateHintType(node.m_sHintType))
			{
				...
				{
					UTIL_TraceLine(pev->origin + pev->view_ofs, node.m_vecOrigin + pev->view_ofs, ignore_monsters,
					               ENT(pev), &tr);

					if (tr.flFraction == 1.0)
					{
						WorldGraph.m_iLastActiveIdleSearch = nodeNumber + 1;
						// next monster that searches for hint nodes will start where we left off.
...
void CBaseMonster::ReportAIState()
{
	...
	if (static_cast<int>(m_MonsterState) < ARRAYSIZE(pStateNames))
...
bool CBaseMonster::CanPlaySequence(int interruptFlags)
{
	if (m_pCine)
	{
		...
		ALERT(at_console, "CANTPLAY: Already playing %s \"%s\"!\n", STRING(m_pCine->pev->classname),
		      STRING(m_pCine->pev->targetname));
#endif
		return false;
	}
	if (!IsAlive() || m_MonsterState == MONSTERSTATE_PRONE)
	{
#ifdef DEBUG_CANTPLAY
...
BOOL CBaseMonster::FindLateralCover(const Vector& vecThreat, const Vector& vecViewOffset)
{
	...
	for (i = 0; i < COVER_CHECKS; i++)
	{
		...
		if (tr.flFraction != 1.0)
		{
			if (FValidateCover(vecLeftTest) && CheckLocalMove(pev->origin, vecLeftTest, nullptr, nullptr) ==
				LOCALMOVE_VALID)
			{
		...
		if (tr.flFraction != 1.0)
		{
			if (FValidateCover(vecRightTest) && CheckLocalMove(pev->origin, vecRightTest, nullptr, nullptr) ==
				LOCALMOVE_VALID
			)
			{
...
CBaseEntity* CBaseMonster::DropItem(const char* pszItemName, const Vector& vecPos, const Vector& vecAng)
{
	...
	CBaseEntity* pItem = Create(pszItemName, vecPos, vecAng, edict());
