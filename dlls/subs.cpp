...
extern BOOL FEntIsVisible(const entvars_t* pev, const entvars_t* pevTarget);
...
void CBaseToggle::LinearMove(Vector vecInput, float flSpeed, float flAccel, float flDecel)
//, BOOL bNow )  // AJH Call this to use acceleration
{
...
void CBaseToggle::LinearMoveNow() // AJH Now supports acceleration
{
	...
	if (m_flLinearAccel > 0 || m_flLinearDecel > 0)
	{
		...
		if (m_bDecelerate == true)
		{
			...
			{
				//	ALERT(at_console, "%s has finished moving\n", STRING(pev->targetname));
		...
		else
		{
			...
			else
			{
				...
				UTIL_SetVelocity(this, vecDestDelta.Normalize() * (m_flLinearMoveSpeed));
				//we are probably going slightly faster.

				// set nextthink to trigger a recall to LinearMoveNow when we need to slow down.
				SetNextThink(
					(vecDestDelta.Length() - (m_flLinearMoveSpeed * m_flDecelTime / 2)) / (m_flLinearMoveSpeed), TRUE);
				SetThink(&CBaseToggle::LinearMoveNow);
