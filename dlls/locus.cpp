...
Vector CalcLocus_Position(CBaseEntity* pEntity, CBaseEntity* pLocus, const char* szText)
{
	if ((*szText >= '0' && *szText <= '9') || *szText == '-')
	{
		// it's a vector
		Vector tmp;
		UTIL_StringToRandomVector(static_cast<float*>(tmp), szText);
		return tmp;
	}
	...
	ALERT(at_error, "%s \"%s\" has bad or missing calc_position value \"%s\"\n", STRING(pEntity->pev->classname),
	      STRING(pEntity->pev->targetname), szText);
	return g_vecZero;
}
...
float CalcLocus_Ratio(CBaseEntity* pLocus, const char* szText, int mode) //AJH added 'mode' = ratio to return
{
	...
	// MJB/AJH - might need to be supressed; this error is often returned when there is no error
...
CBaseEntity* CLocusAlias::FollowAlias(CBaseEntity* pFrom)
{
	if (m_hValue == nullptr)
		return nullptr;
	if (pFrom == nullptr || (OFFSET(m_hValue->pev) > OFFSET(pFrom->pev)))
	{
...
Vector CCalcPosition::CalcPosition(CBaseEntity* pLocus)
{
	...
	case 5:
		...
		static_cast<CBaseAnimating*>(pSubject)->GetAttachment(0, vecPosition, vecJunk);
		return vecOffset + vecPosition;
	case 6:
		static_cast<CBaseAnimating*>(pSubject)->GetAttachment(1, vecPosition, vecJunk);
		return vecOffset + vecPosition;
	case 7:
		static_cast<CBaseAnimating*>(pSubject)->GetAttachment(2, vecPosition, vecJunk);
		return vecOffset + vecPosition;
	case 8:
		static_cast<CBaseAnimating*>(pSubject)->GetAttachment(3, vecPosition, vecJunk);
		return vecOffset + vecPosition;
	case 9:
		return vecOffset + pSubject->pev->origin + Vector(
	...
	if (!FStringNull(pev->noise))
	{
		...
		if (!FStringNull(pev->noise1))
		{
			...
			if (fBasis >= fMin && fBasis <= fMax)
				return fBasis;
			switch (static_cast<int>(pev->frags))
			{
			case 0:
				{
					if (fBasis < fMin)
...
Vector CCalcSubVelocity::CalcVelocity(CBaseEntity* pLocus)
{
	...
	case 5:
		// this could cause problems.
		// is there a good way to check whether it's really a CBaseAnimating?
		static_cast<CBaseAnimating*>(pLocus)->GetAttachment(0, vecJunk, vecAngles);
		return ConvertAngles(pLocus, vecAngles);
	case 6:
		static_cast<CBaseAnimating*>(pLocus)->GetAttachment(1, vecJunk, vecAngles);
		return ConvertAngles(pLocus, vecAngles);
	case 7:
		static_cast<CBaseAnimating*>(pLocus)->GetAttachment(2, vecJunk, vecAngles);
		return ConvertAngles(pLocus, vecAngles);
	case 8:
		static_cast<CBaseAnimating*>(pLocus)->GetAttachment(3, vecJunk, vecAngles);
		return ConvertAngles(pLocus, vecAngles);
	default:
...
Vector CCalcVelocityPath::CalcVelocity(CBaseEntity* pLocus)
{
	...
	switch (static_cast<int>(pev->armorvalue))
	{
	case 0:
	...
	if (pev->health)
	{
		...
		switch (static_cast<int>(pev->health))
		{
		case 1:
			vecOffs = vecOffs / len;
			break;
...
void CLocusVariable::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (m_iszTargetName)
	{
		CMark* pMark = GetClassPtr(static_cast<CMark*>(nullptr));
