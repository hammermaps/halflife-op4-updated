...
int CBasePlayer::TakeArmor(float flArmor)
{
	...
	if (BaseClass::TakeArmor(flArmor))
	{
...
// TraceAttack
//=========================================================
void CBasePlayer::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr,
                              int bitsDamageType)
{
	if (pev->takedamage)
	{
...
void CBasePlayer::PackDeadPlayerItems()
{
			...
			while (pPlayerItem)
			{
				...
				case GR_PLR_DROP_GUN_ACTIVE:
					if (m_pActiveItem && pPlayerItem == m_pActiveItem)
					{
						...
						rgpPackWeapons[iPW++] = static_cast<CBasePlayerWeapon*>(pPlayerItem);
					}
					break;
	...
	CWeaponBox* pWeaponBox = static_cast<CWeaponBox*>(Create("weaponbox", pev->origin, pev->angles, edict()));
...
//LRC
void CBasePlayer::RemoveItems(int iWeaponMask, int i9mm, int i357, int iBuck, int iBolt, int iARGren, int iRock,
                              int iUranium, int iSatchel, int iSnark, int iTrip, int iGren, int iHornet, int i556,
                              int iPenguins, int iShock,
                              int i762, int iSpores)
{
	...
	RemoveAmmo("556", i556);
	RemoveAmmo("Penguins", iPenguins);
	RemoveAmmo("shock", iShock);
	RemoveAmmo("762", i762);
	RemoveAmmo("spores", iSpores);
	...
	for (int i = 0; i < MAX_ITEM_TYPES; i++)
	{
		...
		CBasePlayerItem* pCurrentItem = m_rgpPlayerItems[i];
		while (pCurrentItem->m_pNext)
		{
			if (!(1 << pCurrentItem->m_pNext->m_iId & iWeaponMask))
			{
				static_cast<CBasePlayerWeapon*>(pCurrentItem)->DrainClip(
					this, FALSE, i9mm, i357, iBuck, iBolt, iARGren, iRock, iUranium, iSatchel, iSnark, iTrip, iGren,
					i556, iPenguins, iShock, i762, iSpores);
			...
			else
			{
				...
				static_cast<CBasePlayerWeapon*>(pCurrentItem)->DrainClip(
					this, TRUE, i9mm, i357, iBuck, iBolt, iARGren, iRock, iUranium, iSatchel, iSnark, iTrip, iGren,
					i556, iPenguins, iShock, i762, iSpores);
		...
		{
			static_cast<CBasePlayerWeapon*>(pCurrentItem)->DrainClip(this, FALSE, i9mm, i357, iBuck, iBolt, iARGren,
			                                                         iRock,
			                                                         iUranium, iSatchel, iSnark, iTrip, iGren, i556,
			                                                         iPenguins, iShock, i762, iSpores);
		...
		{
			static_cast<CBasePlayerWeapon*>(pCurrentItem)->DrainClip(this, TRUE, i9mm, i357, iBuck, iBolt, iARGren,
			                                                         iRock, iUranium, iSatchel, iSnark, iTrip, iGren,
			                                                         i556, iPenguins, iShock, i762, iSpores);
			//			ALERT(at_console, "Keeping %s. (id = %d)\n", m_rgpPlayerItems[i]->pszName(), m_rgpPlayerItems[i]->m_iId);
		}
	}
	...
	{
		if (m_pActiveItem && !static_cast<CBasePlayerWeapon*>(m_pActiveItem)->IsUseable())
		{
			//lower the gun if it's out of ammo
			static_cast<CBasePlayerWeapon*>(m_pActiveItem)->m_flTimeWeaponIdle = UTIL_WeaponTimeBase();
		}
...
void CBasePlayer::StartObserver(Vector vecPosition, Vector vecViewAngle)
{
	...
	WRITE_BYTE(static_cast<BYTE>(entindex()));
	MESSAGE_END();
...
void CBasePlayer::PlayerUse()
{
	...
	if (m_afButtonPressed & IN_USE)
	{
		...
		if (pTrain && !(pev->button & IN_JUMP) && FBitSet(pev->flags, FL_ONGROUND) && (pTrain->ObjectCaps() &
			FCAP_DIRECTIONAL_USE) && pTrain->OnControls(pev))
		{
			m_afPhysicsFlags |= PFLAG_ONTRAIN;
...
void CBasePlayer::UpdateGeigerCounter()
{
	...
	range = static_cast<BYTE>(m_flgeigerRange / 4);
...
void CSprayCan::Think()
{
	...
	pPlayer = static_cast<CBasePlayer*>(GET_PRIVATE(pev->owner));
...
void CBasePlayer::ImpulseCommands()
{
	...
	case 201: // paint decal
		...
		if (tr.flFraction != 1.0)
		{
			...
			CSprayCan* pCan = GetClassPtr(static_cast<CSprayCan*>(nullptr));
			pCan->Spawn(pev);
		}
...
void CBasePlayer::CheatImpulseCommands(int iImpulse)
{
	...
	case 202: // Random blood splatter
		...
		{
			// line hit something, so paint a decal
			CBloodSplat* pBlood = GetClassPtr(static_cast<CBloodSplat*>(nullptr));
			pBlood->Spawn(pev);
		}
		break;
...
void CBasePlayer::UpdateClientData()
{
	...
	if (m_fInitHUD)
	{
		...
		MESSAGE_BEGIN(MSG_ONE, gmsgResetMirror, nullptr, pev);
		MESSAGE_END();
		...
		CBaseEntity* pSky = UTIL_FindEntityByClassname(nullptr, "env_sky");
		if (!FNullEnt(pSky))
		{
			MESSAGE_BEGIN(MSG_ONE, gmsgSetSky, nullptr, pev);
			WRITE_BYTE(1); // mode
		...
		pFind = FIND_ENTITY_BY_CLASSNAME(nullptr, "env_mirror");
	...
	if (viewNeedsUpdate != 0)
	{
		...
		CBaseEntity* pViewEnt = UTIL_FindEntityByString(nullptr, "targetname", STRING(viewEntity));
		...
		else
		{
			...
			CBaseEntity* pViewEnt = UTIL_FindEntityByString(nullptr, "classname", STRING(viewEntity));
	...
	{
		int iHealth = clamp(pev->health, 0, std::numeric_limits<short>::max());
...
void CBasePlayer::DropPlayerItem(char* pszItemName)
{
	...
	for (i = 0; i < MAX_ITEM_TYPES; i++)
	{
		...
		if (pWeapon)
		{
			...
			CWeaponBox* pWeaponBox = static_cast<CWeaponBox*>(Create("weaponbox",
...
BOOL CBasePlayer::Menu_Team_Input(int inp)
{
	...
	if (inp <= g_pGameRules->GetNumTeams() && inp > 0)
	{
		...
		if (ctf_autoteam.value == 1)
		{
			...
			{
				if (!g_pGameRules->TeamsBalanced() && inp != g_pGameRules->GetTeamIndex(
					g_pGameRules->TeamWithFewestPlayers()) + 1)
				{
...
TYPEDESCRIPTION CStripWeapons::m_SaveData[] =
{
	...
	DEFINE_FIELD(CStripWeapons, m_i556, FIELD_INTEGER),
	DEFINE_FIELD(CStripWeapons, m_iPenguins, FIELD_INTEGER),
	DEFINE_FIELD(CStripWeapons, m_iShock, FIELD_INTEGER),
	DEFINE_FIELD(CStripWeapons, m_i762, FIELD_INTEGER),
	DEFINE_FIELD(CStripWeapons, m_iSpores, FIELD_INTEGER),
};
...
void CStripWeapons::KeyValue(KeyValueData* pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "saw"))
	{
		m_i556 = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "penguins"))
	{
		m_iPenguins = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "shock"))
	{
		m_iShock = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "sniper"))
	{
		m_i762 = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "spores"))
	{
		m_iSpores = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else
...
void CStripWeapons::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	{
		pPlayer = static_cast<CBasePlayer*>(Instance(g_engfuncs.pfnPEntityOfEntIndex(1)));
	}
	...
	if (pPlayer)
	{
		pPlayer->RemoveItems(pev->spawnflags, m_i9mm, m_i357, m_iBuck, m_iBolt,
		                     ...
		                     m_i556, m_iPenguins, m_iShock, m_i762, m_iSpores);
	}
}
