...
void CController::Spawn()
{
	...
	m_MonsterState = MONSTERSTATE::MONSTERSTATE_NONE;
...
{
	if (pev->model)
		PrecacheModel(STRING(pev->model)); //LRC
	else
...
void CControllerHeadBall::HuntThink()
{
	...
	if ((m_hEnemy->Center() - pev->origin).Length() < 64)
	{
		...
		if (pEntity != nullptr && pEntity->pev->takedamage > 0)
		{
			ClearMultiDamage();
...
void CControllerHeadBall::Crawl()
{
	...
	const Vector vecPnt = pev->origin + pev->velocity * 0.3f + vecAim * 64;
