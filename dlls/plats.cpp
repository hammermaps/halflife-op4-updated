...
void CPlatTrigger :: Touch( CBaseEntity *pOther )
{
	...
	if (platform->m_toggle_state == TS_AT_BOTTOM)
		...
		platform->SetNextThink(1.0);// delay going down
}
...
TYPEDESCRIPTION	CSpriteTrain::m_SaveData[] =
{
	...
	DEFINE_FIELD(CSpriteTrain, m_pSequence, FIELD_CLASSPTR), //LRC
	DEFINE_FIELD(CSpriteTrain, m_vecAvelocity, FIELD_VECTOR), //LRC
};
...
void CSpriteTrain::Spawn()
{
	...
	if (FStringNull(pev->target))
		ALERT(at_console, "env_spritetrain \"%s\" has no target\n", STRING(pev->targetname));
...
//LRC
void CSpriteTrain::ThinkCorrection()
{
	if (m_fStoredThink && pev->nextthink != m_fPevNextThink)
	{
		//		ALERT(at_console, "StoredThink Correction for train \"%s\", %f -> %f\n", STRING(pev->targetname), m_fStoredThink, m_fStoredThink + pev->nextthink - m_fPevNextThink);
...
//LRC
void CSpriteTrain::StartSequence(CTrainSequence* pSequence)
{
	m_pSequence = pSequence;
	//	ALERT(at_console, "Unset Retrigger (startsequence)\n");
	pev->spawnflags &= ~SF_TRAIN_WAIT_RETRIGGER;
...
//LRC
void CSpriteTrain::StopSequence()
{
	m_pSequence = NULL;
	//	pev->spawnflags &= ~SF_TRAIN_WAIT_RETRIGGER;
	pev->spawnflags &= ~SF_TRAIN_REVERSE;
